...
import { rowssmie } from "./constant-etatmateriel";
import { Box, Checkbox, IconButton, Paper, styled, Stack } from "@mui/material";
import EtatMaterielTableHeader from "./deviseTableHeader";
...
const ListDevise = () => {
  ...
  const [orderBy, setOrderBy] = React.useState<keyof DataEtatMateriel>("devise");
  ...
  const handleSelectAllClick = (event: React.ChangeEvent<HTMLInputElement>) => {
    if (event.target.checked) {
      const newSelecteds = rowssmie.map((n) => n.devise);
  ...
  const emptyRows =
    page > 0 ? Math.max(0, (1 + page) * rowsPerPage - rowssmie.length) : 0;
